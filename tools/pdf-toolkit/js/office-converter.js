...
(() => {
  ...
  async function extractVectorTableGrid(page) {
    // Final vector-first table detector. It handles both rectangle borders and
    // line/path borders, and it tracks the PDF graphics transform so the
    // geometry is interpreted in the same coordinate system as PDF.js text.
    ...
    try {
      ...
      const toViewport = (x, y) => {
        ...
        return { x, y: viewport.height - y };
      };
      const addSegment = (p1, p2) => {
        const a = toViewport(p1.x, p1.y);
        const b = toViewport(p2.x, p2.y);
        const dx = b.x - a.x;
        const dy = b.y - a.y;
        const len = Math.hypot(dx, dy);
        if (!Number.isFinite(len) || len < 8) return;
        const horizontal = Math.abs(dy) <= 2.2 && Math.abs(dx) >= 8;
        const vertical = Math.abs(dx) <= 2.2 && Math.abs(dy) >= 8;
        if (!horizontal && !vertical) return;
        segments.push({ x0: Math.min(a.x, b.x), x1: Math.max(a.x, b.x), y0: Math.min(a.y, b.y), y1: Math.max(a.y, b.y), horizontal, vertical, len });
      };

      let ctm = identity;
      const stack = [];
      const opRectangle = OPS.rectangle;
      const opTransform = OPS.transform;
      const opSave = OPS.save;
      const opRestore = OPS.restore;
      const opConstructPath = OPS.constructPath;
      const opMoveTo = OPS.moveTo;
      const opLineTo = OPS.lineTo;
      const opClosePath = OPS.closePath;

      for (let i = 0; i < opList.fnArray.length; i += 1) {
        const fn = opList.fnArray[i];
        const args = opList.argsArray[i];

        if (fn === opSave) { stack.push(ctm.slice()); continue; }
        if (fn === opRestore) { ctm = stack.pop() || identity; continue; }
        if (fn === opTransform && Array.isArray(args) && args.length >= 6) {
          const n = args.slice(0, 6).map(Number);
          if (n.every(Number.isFinite)) ctm = multiply(ctm, n);
          continue;
        }

        if (fn === opRectangle && Array.isArray(args) && args.length >= 4) {
          const x = Number(args[0]), y = Number(args[1]), w = Number(args[2]), h = Number(args[3]);
          ...
          for (let j = 0; j < 4; j += 1) addSegment(p[j], p[(j + 1) % 4]);
          continue;
        }

        // Some PDFs draw table borders with constructPath instead of rectangle.
        if (fn === opConstructPath && Array.isArray(args)) {
          const ops = Array.isArray(args[0]) ? args[0] : [];
          const vals = Array.isArray(args[1]) ? args[1] : [];
          let vi = 0;
          let current = null;
          let subStart = null;
          const pathMove = (x, y) => { const q = apply(ctm, Number(x), Number(y)); current = q; subStart = q; };
          for (const pathOp of ops) {
            if (pathOp === opMoveTo) {
              if (vi + 1 < vals.length) { pathMove(vals[vi], vals[vi + 1]); vi += 2; }
            } else if (pathOp === opLineTo) {
              if (vi + 1 < vals.length && current) {
                const q = apply(ctm, Number(vals[vi]), Number(vals[vi + 1]));
                addSegment(current, q); current = q; vi += 2;
              }
            } else if (pathOp === opClosePath) {
              if (current && subStart) addSegment(current, subStart);
              current = subStart;
            } else {
              // curveTo consumes six values; quadratic and other path ops are
              // not useful for table borders, but consuming their arguments
              // keeps the parser aligned for the common PDF.js op arrays.
              if (pathOp === OPS.curveTo || pathOp === OPS.curveTo2 || pathOp === OPS.curveTo3) vi += 6;
            }
          }
        }
      }

      // Convert thin segments into the same normalized border representation.
      for (const s of segments) {
        if (s.vertical) rectangles.push({ x0: s.x0, x1: s.x1, y0: s.y0, y1: s.y1, width: s.x1 - s.x0, height: s.y1 - s.y0 });
        else if (s.horizontal) rectangles.push({ x0: s.x0, x1: s.x1, y0: s.y0, y1: s.y1, width: s.x1 - s.x0, height: s.y1 - s.y0 });
      }

      if (rectangles.length < 6) return null;
      ...
      const cluster = (values, tolerance) => {
        ...
        for (const value of sorted) {
          ...
          if (!last || Math.abs(value - last.mean) > tolerance) out.push({ mean: value, values: [value] });
          else { last.values.push(value); last.mean = last.values.reduce((sum, n) => sum + n, 0) / last.values.length; }
        }
        return out;
      };

      const vertical = rectangles.filter(r => r.width <= 3.0 && r.height >= 8);
      const horizontal = rectangles.filter(r => r.height <= 3.0 && r.width >= 8);
      if (vertical.length < 3 || horizontal.length < 2) return null;

      const xClusters = cluster(vertical.map(r => (r.x0 + r.x1) / 2), 2.5);
      const yClusters = cluster(horizontal.map(r => (r.y0 + r.y1) / 2), 2.5);
      if (xClusters.length < 3) return null;

      const xLines = xClusters.map(c => {
        const parts = vertical.filter(r => Math.abs((r.x0 + r.x1) / 2 - c.mean) <= 2.5);
        return { x: c.mean, minY: Math.min(...parts.map(r => r.y0)), maxY: Math.max(...parts.map(r => r.y1)), count: parts.length };
      }).filter(v => v.count >= 1).sort((a, b) => a.x - b.x);

      // Prefer a 5-line grid when present (four columns), but remain generic:
      // score candidate boundary sets by vertical overlap, width, and coverage.
      let best = null;
      for (let a = 0; a < xLines.length - 2; a += 1) {
        for (let b = a + 2; b < xLines.length; b += 1) {
          const subset = xLines.slice(a, b + 1);
          const minY = Math.max(...subset.map(v => v.minY));
          const maxY = Math.min(...subset.map(v => v.maxY));
          const width = subset.at(-1).x - subset[0].x;
          const height = maxY - minY;
          if (width < 80 || height < 20) continue;
          const coverage = Math.min(1, height / Math.max(1, viewport.height * 0.45));
          const score = subset.length * width * Math.max(1, height) * (1 + coverage);
          if (!best || score > best.score) best = { subset, minY, maxY, score };
        }
      }
      if (!best || best.subset.length < 3) return null;

      const left = best.subset[0].x;
      const right = best.subset.at(-1).x;
      const yLines = yClusters.map(c => {
        const parts = horizontal.filter(r => Math.abs((r.y0 + r.y1) / 2 - c.mean) <= 2.5);
        return { y: c.mean, minX: Math.min(...parts.map(r => r.x0)), maxX: Math.max(...parts.map(r => r.x1)), span: Math.max(...parts.map(r => r.x1)) - Math.min(...parts.map(r => r.x0)) };
      }).filter(r => r.minX <= left + 10 && r.maxX >= right - 10).sort((a, b) => a.y - b.y);

      const rowYs = [];
      const pushY = y => { if (!rowYs.length || Math.abs(y - rowYs.at(-1)) > 3) rowYs.push(y); };
      pushY(best.minY);
      for (const line of yLines) if (line.y >= best.minY - 4 && line.y <= best.maxY + 4) pushY(line.y);
      pushY(best.maxY);
      if (rowYs.length < 2) return null;

      return {
        xLines: best.subset.map(v => v.x),
        yLines: rowYs,
        xMin: left,
        xMax: right,
        yMin: rowYs[0],
        yMax: rowYs.at(-1),
        confidence: 0.99,
        geometrySource: 'pdf-vector-paths'
      };
    } catch (_) {
      return null;
    }
  ...
  function detectGridTable(lines, pageNumber, pageWidth, pageHeight, grid) {
    ...
    for (let r = 0; r < y.length - 1; r += 1) {
      ...
      const cells = Array.from({ length: x.length - 1 }, () => []);
      const cellItems = Array.from({ length: x.length - 1 }, () => []);

      for (const line of lines) {
        if (line.yBottom < top - 2 || line.yTop > bottom + 2) continue;
        for (const item of (line.items || [])) {
          const centerX = Number(item.x) + Number(item.width || 0) / 2;
          const centerY = Number(line.baseline);
          if (!Number.isFinite(centerX) || !Number.isFinite(centerY)) continue;
          if (centerX < x[0] - 2 || centerX > x[x.length - 1] + 2) continue;
          if (centerY < top - 3 || centerY > bottom + 3) continue;

          let col = -1;
          for (let c = 0; c < x.length - 1; c += 1) {
            if (centerX >= x[c] - 2 && centerX <= x[c + 1] + 2) {
              col = c;
              break;
            }
          }
          if (col < 0) continue;
          cellItems[col].push({ item, line });
        }
      }

      for (let c = 0; c < cellItems.length; c += 1) {
        const ordered = cellItems[c].sort((a, b) =>
          a.line.yTop - b.line.yTop || a.item.x - b.item.x
        );
        const pieces = [];
        let currentY = null;
        let currentLine = [];
        ...
        const flushLine = () => {
          if (currentLine.length) {
            pieces.push(lineJoin(currentLine.sort((a, b) => a.x - b.x)));
            currentLine = [];
          }
        };
        ...
        for (const entry of ordered) {
          ...
          if (currentY === null || Math.abs(entry.line.baseline - currentY) <= tolerance) {
            currentLine.push(item);
            currentY = currentY === null ? entry.line.baseline : (currentY + entry.line.baseline) / 2;
          } else {
            flushLine();
            currentLine.push(item);
            currentY = entry.line.baseline;
          }
        }
        flushLine();
        cells[c] = pieces.filter(Boolean);
      }

      const values = cells.map(parts => normalizeText(parts.join(' ')));
      const lineIndexes = [...new Set(
        cellItems.flatMap(items => items.map(entry => entry.line.index))
      )];

      rows.push({ cells: values, lineIndexes, yTop: top, yBottom: bottom });
    }
    ...
    const density = populated / (meaningful.length * (x.length - 1));
    ...
    return {
      id: `vector-table-${pageNumber}`,
      pageNumber,
      title: '',
      rows: meaningful.map(row => row.cells),
      confidence: Math.min(0.99, 0.88 + Math.min(0.10, density * 0.10)),
      lineIndexes: [...new Set(meaningful.flatMap(row => row.lineIndexes))],
...
function detectTables(lines, pageNumber, pageWidth, options = {}, pageHeight = 792, vectorGrid = null) {
  // Coordinate-first table reconstruction. PDF.js groups text that shares a
  // baseline into one visual line, even when that line contains several table
  // columns. Therefore table detection must inspect line.items/segments rather
  // than treating line.x as the row's only position.
  if (!Array.isArray(lines) || lines.length < 1) return [];

  const mode = options.mode || 'document';
  const gridTable = detectGridTable(lines, pageNumber, pageWidth, pageHeight, vectorGrid);
  ...
  // Stable column anchors come from the LEFT EDGE of visual segments.
  // A segment is normally one logical cell fragment because segmentLine()
  // splits at unusually large horizontal gaps.
  ...
  for (const x of ordered) {
    ...
    if (!last || Math.abs(x - last.mean) > tolerance) {
      clusters.push({ mean: x, count: 1, values: [x] });
    } else {
      last.values.push(x);
      last.count++;
  ...
  // A real table generally has at least three stable vertical anchors. This
  // deliberately avoids converting ordinary two-column prose into a table.
  ...
  // Require a substantial horizontal span. Narrow repeated alignments are
  // more likely to be indented paragraphs/lists than a table.
  if (anchors[anchors.length - 1] - anchors[0] < pageWidth * 0.35) return [];

  // Map each segment to the nearest stable anchor.
  const mapped = visual.map(v => ({
    ...v,
    cells: v.segments.map(seg => {
      let best = 0, distance = Infinity;
      anchors.forEach((x, i) => {
        const d = Math.abs(seg.x - x);
        if (d < distance) { distance = d; best = i; }
      });
      return { ...seg, column: best, distance };
    })
  }));

  // Estimate normal line spacing. A large vertical jump is a strong signal
  // that a new logical table row has begun, while small jumps are wrapped text
  // inside the same row.
  const ys = mapped.map(v => v.line.yTop).sort((a,b) => a-b);
  const gaps = [];
  ...
  const medianGap = sortedGaps.length
    ...
    : 10;
  const rowBreakGap = Math.max(medianGap * 1.45, medianGap + 3);

  const rowGroups = [];
  let current = null;
  for (const item of mapped) {
    if (!current) {
      current = { lines: [item], yTop: item.line.yTop, yBottom: item.line.yBottom };
      continue;
    }
    const gap = item.line.yTop - current.lines[current.lines.length - 1].line.yBottom;
    if (gap > rowBreakGap) {
      rowGroups.push(current);
      current = { lines: [item], yTop: item.line.yTop, yBottom: item.line.yBottom };
    } else {
      current.lines.push(item);
      current.yBottom = Math.max(current.yBottom, item.line.yBottom);
    }
  }
  if (current) rowGroups.push(current);

  // Collapse rows that do not participate in the stable column pattern.
  const tableRows = rowGroups.map(group => {
    const cells = Array.from({length: anchors.length}, () => []);
    const lineIndexes = [];
    for (const visualLine of group.lines) {
      lineIndexes.push(visualLine.line.index);
      for (const cell of visualLine.cells) {
        if (cell.distance <= Math.max(18, pageWidth * 0.035)) {
          cells[cell.column].push(cell.text);
        }
      }
    }
    return {
      cells: cells.map(parts => normalizeText(parts.join(' '))),
      lineIndexes,
      yTop: group.yTop,
  ...
  // A table should have meaningful multi-column occupancy. For one tall row
  // (common in forms), require 3+ populated columns. For multi-row tables,
  // require repeated occupancy in the same columns.
  const occupancy = anchors.map((_, col) => tableRows.filter(r => r.cells[col]).length);
  const populatedColumns = occupancy.filter(n => n > 0).length;
  const repeatedColumns = occupancy.filter(n => n >= Math.max(2, Math.ceil(tableRows.length * 0.18))).length;

  if (populatedColumns < 3) return [];
  if (tableRows.length > 1 && repeatedColumns < 3) return [];

  const populatedCells = tableRows.reduce((sum, row) => sum + row.cells.filter(Boolean).length, 0);
  const density = populatedCells / (tableRows.length * anchors.length);

  // Avoid giant false-positive "tables" made from normal multi-column prose.
  // A genuine form/table normally has either repeated rows or very compact
  // right/left cells alongside a wider middle column.
  const hasRepeatedRows = tableRows.length >= 2;
  const compactEdge = tableRows.some(row =>
    row.cells[0] && row.cells[0].length < 80 &&
    row.cells[anchors.length - 1] && row.cells[anchors.length - 1].length < 100
  );
  if (!hasRepeatedRows && !compactEdge) return [];
  if (density < (sensitivity === 'aggressive' ? 0.28 : 0.34)) return [];

  const columnWidths = anchors.map((x, i) => {
    const left = i === 0 ? anchors[0] : (anchors[i-1] + x) / 2;
    const right = i === anchors.length - 1
      ? pageWidth - (pageWidth - anchors[anchors.length - 1]) * 0.55
      : (x + anchors[i+1]) / 2;
    return Math.max(2, right - left);
  });
  const widthTotal = columnWidths.reduce((a,b)=>a+b,0);
  const widthPercent = columnWidths.map(w => Math.max(2, Math.min(70, w / widthTotal * 100)));

  return [{
    id: `spatial-table-${pageNumber}-0`,
    pageNumber,
    title: '',
    rows: tableRows.map(r => r.cells),
    confidence: Math.min(0.99, 0.60 + Math.min(0.25, density * 0.2) + (hasRepeatedRows ? 0.10 : 0.04)),
    lineIndexes: [...new Set(tableRows.flatMap(r => r.lineIndexes))],
    yTop: tableRows[0].yTop,
    yBottom: tableRows[tableRows.length - 1].yBottom,
    columns: anchors,
    columnWidths: widthPercent
  }];
}
  ...
  function createPageBlocks(page, options) {
    ...
    classifyLines(lines);
    const tables = detectTables(lines, page.pageNumber, page.width, options, page.height, page.vectorGrid);
    ...
    return new Table({
      ...
      rows: rows.map((row, rowIndex) => new TableRow({
        ...
        children: row.map((cell, cellIndex) => new TableCell({
          ...
          width: table.columnWidths?.[cellIndex] ? { size: table.columnWidths[cellIndex], type: WidthType.PERCENTAGE } : undefined,
