...
(() => {
  ...
  function detectTables(lines, pageNumber) {
  // Generic spatial table reconstruction. This deliberately avoids document-specific
  // headers/column names. It uses repeated x-anchors, row alignment, and compact
  // vertical spacing to identify tabular regions.
  ...
  const usable = lines
    .map((line, index) => ({
      ...line,
      _index: index,
      _text: String(line.text || '').trim(),
      _x: Number(line.x ?? line.minX ?? 0),
      _y: Number(line.y ?? line.top ?? 0),
      _right: Number(line.right ?? line.maxX ?? (Number(line.x ?? 0) + Number(line.width ?? 0)))
    }))
    .filter(l => l._text && Number.isFinite(l._x) && Number.isFinite(l._y));

  if (usable.length < 3) return [];

  // Cluster nearby left edges into recurring column anchors.
  const sortedX = [...new Set(usable.map(l => Math.round(l._x)))].sort((a,b) => a-b);
  const anchors = [];
  for (const x of sortedX) {
    const last = anchors[anchors.length - 1];
    if (!last || x - last.center > 18) anchors.push({center:x, count:1});
    else {
  ...
  const strongAnchors = anchors.filter(a => a.count >= 2);
  if (strongAnchors.length < 2) return [];

  // Build candidate rows by Y proximity.
  const rows = [];
  const yTolerance = 5;
  for (const line of usable.sort((a,b) => a._y - b._y || a._x - b._x)) {
    let row = rows.find(r => Math.abs(r.y - line._y) <= yTolerance);
    if (!row) {
      row = { y: line._y, lines: [] };
      rows.push(row);
    }
  ...
  for (const row of rows) {
    ...
    for (const line of row.lines) {
      ...
      strongAnchors.forEach((a, i) => {
        const d = Math.abs(line._x - a.center);
        if (d < dist) { dist = d; best = i; }
      });
      if (best >= 0 && dist <= 28) occupied.add(best);
    }

    // A row is table-like when multiple recurring anchors are populated.
    const isTableRow = occupied.size >= 2;
    if (isTableRow) {
      run.push({ row, occupied });
    } else {
      flush();
    }
  }
  ...
  return candidates.map((run, candidateIndex) => {
    const colCount = Math.max(...run.map(r => r.occupied.size));
    if (colCount < 2) return null;

    const usedAnchors = strongAnchors
      .filter((a, i) => run.some(r => r.occupied.has(i)))
      .map(a => a.center);

    if (usedAnchors.length < 2) return null;

    // Merge very close anchors and create stable boundaries.
    const cols = [];
    for (const x of usedAnchors.sort((a,b) => a-b)) {
      if (!cols.length || x - cols[cols.length - 1] > 25) cols.push(x);
    }
    if (cols.length < 2) return null;

    const tableRows = run.map(r => {
      const cells = Array.from({length: cols.length}, () => []);
      for (const line of r.row.lines) {
        let ci = 0, dist = Infinity;
        cols.forEach((x, i) => {
          const d = Math.abs(line._x - x);
          if (d < dist) { dist = d; ci = i; }
        });
        cells[ci].push(line._text);
      }
      return cells.map(parts => parts.join(' ').replace(/\s+/g, ' ').trim());
    });

    const populated = tableRows.flat().filter(Boolean).length;
    const density = populated / (tableRows.length * cols.length);
    const rowCount = tableRows.length;

    // Conservative validation to avoid converting ordinary paragraphs into tables.
    if (rowCount < 2 || density < 0.45) return null;
    if (cols.length === 2 && tableRows.every(r => r.filter(Boolean).length < 2)) return null;

    return {
      id: `spatial-table-${pageNumber}-${candidateIndex}`,
      pageNumber,
      title: '',
      rows: tableRows,
      confidence: Math.min(0.99, 0.55 + density * 0.35 + Math.min(cols.length, 5) * 0.02),
      lineIndexes: run.flatMap(r => r.row.lines.map(l => l._index)),
      yTop: run[0].row.y,
      yBottom: run[run.length - 1].row.y,
      columns: cols
    };
  }).filter(Boolean);
}
