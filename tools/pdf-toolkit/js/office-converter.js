(() => {
  ...
  function detectWrappedTables(lines, pageNumber, pageWidth, options) {
    ...
    for (let i = headerIndex + 1; i < lines.length; i += 1) {
      const line = lines[i];
      if (line.yTop < lines[headerIndex].yTop) continue;
      if (line.kind?.startsWith('heading') && rows.length >= 1) break;
      const items = [...(line.items || [])].sort((a, b) => a.x - b.x);
      if (!items.length) continue;

      const firstItem = items[0];
      const startsNumberedRow = /^\d+[.)]?$/.test(normalizeText(firstItem.text)) &&
        Math.abs(firstItem.x - anchors[0]) <= Math.max(tolerance, 18);

      if (startsNumberedRow) {
        if (current) rows.push(current);
        current = ['', '', '', ''];
        usedLineIndexes.push(line.index);
      } else if (!current) {
        // Ignore text between the header and the first data row.
        continue;
      } else {
        usedLineIndexes.push(line.index);
      }
      ...
      // item-based rather than line-based, so wrapped cells retain their column.
      for (const item of items) append(columnForX(item.x), item.text);

      // A new numbered section after several rows marks the end of this table.
      if (current && current[0] && !/^\d+$/.test(current[0])) break;
    }
    if (current) rows.push(current);
