...
(() => {
  ...
  async function extractVectorTableGrid(page) {
    // Many real PDFs contain the table itself as vector border geometry.
    // When that geometry exists, it is a much stronger signal than text
    // heuristics: the borders tell us the exact columns and row bands.
    ...
    try {
      ...
      const rectangles = [];
      const pageHeight = page.viewBox ? Number(page.viewBox[3] - page.viewBox[1]) : page.getViewport({ scale: 1 }).height;

      for (let i = 0; i < opList.fnArray.length; i += 1) {
        const fn = opList.fnArray[i];
        const args = opList.argsArray[i];

        if (fn !== OPS.rectangle || !Array.isArray(args) || args.length < 4) continue;
        ...
        if (Math.abs(w) > 20 || Math.abs(h) > 20) {
          // Table borders are normally thin rectangles. Ignore large filled
          // graphics/backgrounds here.
          continue;
        }

        const x0 = Math.min(x, x + w);
        const x1 = Math.max(x, x + w);
        const rawY0 = Math.min(y, y + h);
        const rawY1 = Math.max(y, y + h);

        // PDF user space is bottom-up; text extraction is top-down.
        const y0 = pageHeight - rawY1;
        const y1 = pageHeight - rawY0;

        const width = x1 - x0;
        const height = y1 - y0;
        if ((width <= 1.8 && height >= 8) || (height <= 1.8 && width >= 8)) {
          rectangles.push({ x0, x1, y0, y1, width, height });
        }
      }
      ...
      const cluster = (values, tolerance) => {
        ...
        for (const value of sorted) {
          ...
          if (!last || Math.abs(value - last.mean) > tolerance) {
            out.push({ mean: value, values: [value] });
          } else {
      ...
      const vertical = rectangles.filter(r => r.width <= 1.8 && r.height >= 8);
      const horizontal = rectangles.filter(r => r.height <= 1.8 && r.width >= 8);

      const xClusters = cluster(
        vertical.flatMap(r => [(r.x0 + r.x1) / 2]),
        2.5
      );
      const yClusters = cluster(
        horizontal.flatMap(r => [(r.y0 + r.y1) / 2]),
        2.5
      );

      if (xClusters.length < 3) return null;

      const xLines = xClusters
        .map(c => {
          const x = c.mean;
          const parts = vertical.filter(r => Math.abs((r.x0 + r.x1) / 2 - x) <= 2.5);
          return {
            x,
            minY: Math.min(...parts.map(r => r.y0)),
            maxY: Math.max(...parts.map(r => r.y1)),
            count: parts.length
          };
        })
        .filter(v => v.count >= 1)
      ...
      // Find the largest coherent set of vertical boundaries that overlap
      // vertically. This avoids unrelated form graphics elsewhere on a page.
      let best = null;
      for (let a = 0; a < xLines.length; a += 1) {
        for (let b = a + 2; b < xLines.length; b += 1) {
          ...
          const score = width * Math.min(height, pageHeight) * subset.length;
      ...
      const right = best.subset[best.subset.length - 1].x;
      const tableWidth = right - left;

      // Horizontal rules must span most of the detected table width. Short
      // horizontal graphics inside a cell are not row boundaries.
      const yLines = yClusters
        .map(c => {
          const y = c.mean;
          const parts = horizontal.filter(r => Math.abs((r.y0 + r.y1) / 2 - y) <= 2.5);
          const minX = Math.min(...parts.map(r => r.x0));
          const maxX = Math.max(...parts.map(r => r.x1));
          return { y, minX, maxX, span: maxX - minX };
        })
        .filter(r => r.minX <= left + 8 && r.maxX >= right - 8)
        .sort((a, b) => a.y - b.y);

      // The vertical borders establish the table even when horizontal rules
      // are sparse. Add the top/bottom of the vertical region as row limits.
      const rowYs = [];
      const pushY = (y) => {
        if (!rowYs.length || Math.abs(y - rowYs[rowYs.length - 1]) > 3) rowYs.push(y);
      };
      pushY(best.minY);
      for (const line of yLines) {
        if (line.y >= best.minY - 3 && line.y <= best.maxY + 3) pushY(line.y);
      }
      pushY(best.maxY);
    ...
    for (let r = 0; r < y.length - 1; r += 1) {
      ...
      for (const line of lines) {
        ...
        for (const item of (line.items || [])) {
          ...
          const centerY = Number(item.y) + Number(item.height || line.height || 0) / 2;
